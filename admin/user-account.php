  ...
  if (mysqli_num_rows($check_query) > 0) {
      ...
      if (mysqli_query($db_conn, $query)) {
          ...
          if ($type == 'teacher') {
              ...
              if (!isset($_POST['qualification']) || !isset($_POST['department']) || !isset($_POST['experience'])) {
                  die('Teacher-specific fields are missing.');
              }
              $qualification = mysqli_real_escape_string($db_conn, $_POST['qualification']);
              $department    = mysqli_real_escape_string($db_conn, $_POST['department']);
              $experience    = mysqli_real_escape_string($db_conn, $_POST['experience']);

              $teacher_query = "INSERT INTO teachers (account_id, qualification, department, experience) VALUES ('$account_id', '$qualification', '$department', '$experience')";
              mysqli_query($db_conn, $teacher_query) or die(mysqli_error($db_conn));
          } elseif ($type == 'student') {
...
if (isset($_GET['action']) && $_GET['action'] == 'delete' && isset($_GET['id'])) {
  ...
  header('Location: user-account.php?user=' . $_REQUEST['user']);
  exit;
}
?>
